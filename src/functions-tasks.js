...
function getPolynom(...args) {
  ...
  return function polynom(x) {
    ...
    return args.reduce((accumulator, coeff, index) => {
      return accumulator + coeff * x ** (size - index);
    }, 0);
  };
}
...
function logger(func, logFunc) {
  return (...args) => {
    const argum = JSON.stringify(args);
    logFunc(`${func.name}(${argum.slice(1, argum.length - 1)}) starts`);
    const result = func(...args);
    logFunc(`${func.name}(${argum.slice(1, argum.length - 1)}) ends`);
    return result;
  };
}
